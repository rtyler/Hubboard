...
smalltalk.send(self['@userApi'], "_fetchCurrent_", [(function(data){(self['@userData']=data);smalltalk.send(smalltalk.send(unescape("%23logout-username"), "_asJQuery", []), "_text_", [smalltalk.send(smalltalk.send(unescape("%28"), "__comma", [smalltalk.send(data, "_at_", ["login"])]), "__comma", [unescape("%29")])]);return smalltalk.send((smalltalk.Repo || Repo), "_reposFor_with_", [smalltalk.send(data, "_at_", ["login"]), (function(results){return smalltalk.send(results, "_do_", [(function(result){return smalltalk.send(result, "_at_ifTrue_", ["has_issues", (function(){var owner=nil;
(owner=smalltalk.send(smalltalk.send(result, "_at_", ["owner"]), "_at_", ["login"]));return smalltalk.send(self['@knownRepos'], "_add_", [smalltalk.send(smalltalk.send(owner, "__comma", [unescape("/")]), "__comma", [smalltalk.send(result, "_at_", ["name"])])]);})]);})]);})]);})]);
...
return self;},
args: [],
source: unescape('bootstrap%0A%09issueApi%20%3A%3D%20Issues%20new%20setToken%3A%20token.%0A%09userApi%20%3A%3D%20Users%20new%20setToken%3A%20token.%0A%09self%20showSpinner.%0A%09userApi%20fetchCurrent%3A%20%5B%20%3Adata%20%7C%0A%09%09userData%20%3A%3D%20data.%0A%09%09%27%23logout-username%27%20asJQuery%20text%3A%20%28%27%28%27%2C%20%28data%20at%3A%20%27login%27%29%2C%20%27%29%27%29.%0A%09%09%22%20Once%20we%20have%20information%20about%20the%20user%2C%20let%27s%20fire%20up%20our%20repo%20backfill%20%22%09%0A%09%09Repo%20reposFor%3A%20%28data%20at%3A%20%27login%27%29%20with%3A%20%5B%20%3Aresults%20%7C%0A%09%09%09results%20do%3A%20%5B%20%3Aresult%20%7C%0A%09%09%09%09result%20at%3A%20%27has_issues%27%20ifTrue%3A%20%5B%20%0A%09%09%09%09%09%7C%20owner%20%7C%0A%09%09%09%09%09owner%20%3A%3D%20%28%28result%20at%3A%20%27owner%27%29%20at%3A%20%27login%27%29.%0A%09%09%09%09%09knownRepos%20add%3A%20owner%2C%20%27/%27%2C%20%28result%20at%3A%20%27name%27%29.%0A%09%09%09%09%5D.%0A%09%09%5D%5D.%0A%09%5D.%0A%09self%20refresh.%0A%09self%20startRefreshTimer.%0A%09%27.issuecolumn%27%20asJQuery%20droppable%3A%20%23%7B%0A%09%09%09%27tolerance%27%20-%3E%20%27pointer%27.%0A%09%09%09%27accept%27%20-%3E%20%27.issuetile%27.%0A%09%09%09%27drop%27%20-%3E%20%5B%20%3Aevent%20%3Aui%20%7C%20self%20handleDrop%3A%20event%20with%3A%20ui%5D%7D.'),
messageSends: ["setToken:", "new", "showSpinner", "fetchCurrent:", "text:", "asJQuery", unescape("%2C"), "at:", "reposFor:with:", "do:", "at:ifTrue:", "add:", "refresh", "startRefreshTimer", "droppable:", unescape("-%3E"), "handleDrop:with:"],
referencedClasses: ["Issues", "Users", "Repo"]
}),
smalltalk.HubboardApp);
...
(function($rec){smalltalk.send($rec, "_at_put_", ["title", "Create a new issue"]);smalltalk.send($rec, "_id_", [self['@elementId']]);return smalltalk.send($rec, "_with_", [(function(){return (function($rec){smalltalk.send($rec, "_name_", ["new_issue_form"]);smalltalk.send($rec, "_onSubmit_", [(function(event){smalltalk.send(self, "_submit", []);return smalltalk.send(event, "_preventDefault", []);})]);return smalltalk.send($rec, "_with_", [(function(){(function($rec){smalltalk.send($rec, "_for_", ["assignee"]);return smalltalk.send($rec, "_with_", ["Assign to: "]);})(smalltalk.send(html, "_label", []));(function($rec){smalltalk.send($rec, "_name_", ["assignee"]);return smalltalk.send($rec, "_with_", [(function(){return (function($rec){smalltalk.send($rec, "_value_", [currentUser]);return smalltalk.send($rec, "_with_", [currentUser]);})(smalltalk.send(html, "_option", []));})]);})(smalltalk.send(html, "_select", []));smalltalk.send(html, "_br", []);(function($rec){smalltalk.send($rec, "_for_", ["project"]);return smalltalk.send($rec, "_with_", ["Project: "]);})(smalltalk.send(html, "_label", []));(function($rec){smalltalk.send($rec, "_name_", ["project"]);smalltalk.send($rec, "_onChange_", [(function(event){return smalltalk.send(self, "_updateCollaborators", []);})]);return smalltalk.send($rec, "_with_", [(function(){return smalltalk.send(smalltalk.send(smalltalk.send((smalltalk.HubboardApp || HubboardApp), "_current", []), "_knownRepos", []), "_do_", [(function(repo){return ((($receiver = smalltalk.send(smalltalk.send(smalltalk.send((smalltalk.HubboardApp || HubboardApp), "_current", []), "_currentProject", []), "__eq", [repo])).klass === smalltalk.Boolean) ? ($receiver ? (function(){return (function($rec){smalltalk.send($rec, "_value_", [repo]);smalltalk.send($rec, "_with_", [repo]);return smalltalk.send($rec, "_at_put_", ["selected", "true"]);})(smalltalk.send(html, "_option", []));})() : (function(){return (function($rec){smalltalk.send($rec, "_value_", [repo]);return smalltalk.send($rec, "_with_", [repo]);})(smalltalk.send(html, "_option", []));})()) : smalltalk.send($receiver, "_ifTrue_ifFalse_", [(function(){return (function($rec){smalltalk.send($rec, "_value_", [repo]);smalltalk.send($rec, "_with_", [repo]);return smalltalk.send($rec, "_at_put_", ["selected", "true"]);})(smalltalk.send(html, "_option", []));}), (function(){return (function($rec){smalltalk.send($rec, "_value_", [repo]);return smalltalk.send($rec, "_with_", [repo]);})(smalltalk.send(html, "_option", []));})]));})]);})]);})(smalltalk.send(html, "_select", []));smalltalk.send(html, "_br", []);(function($rec){smalltalk.send($rec, "_for_", ["title"]);return smalltalk.send($rec, "_with_", ["Title: "]);})(smalltalk.send(html, "_label", []));(function($rec){smalltalk.send($rec, "_name_", ["title"]);return smalltalk.send($rec, "_at_put_", ["size", "40"]);})(smalltalk.send(html, "_input", []));smalltalk.send(html, "_br", []);(function($rec){smalltalk.send($rec, "_with_", ["Add body"]);smalltalk.send($rec, "_class_", [unescape("dialog-add-body")]);return smalltalk.send($rec, "_onClick_", [(function(){return smalltalk.send(smalltalk.send(unescape("%23dialog-body"), "_asJQuery", []), "_show", []);})]);})(smalltalk.send(html, "_a", []));smalltalk.send(html, "_br", []);(function($rec){smalltalk.send($rec, "_id_", [unescape("dialog-body")]);smalltalk.send($rec, "_style_", [unescape("display%3A%20none%3B")]);return smalltalk.send($rec, "_with_", [(function(){smalltalk.send(html, "_with_", [(function($rec){smalltalk.send($rec, "_setName_", ["body"]);smalltalk.send($rec, "_setColumns_", [(40)]);return smalltalk.send($rec, "_setRows_", [(8)]);})(smalltalk.send((smalltalk.MarkdownTextArea || MarkdownTextArea), "_new", []))]);return smalltalk.send(html, "_br", []);})]);})(smalltalk.send(html, "_div", []));return (function($rec){smalltalk.send($rec, "_style_", [unescape("float%3A%20right%3B")]);smalltalk.send($rec, "_type_", ["submit"]);return smalltalk.send($rec, "_with_", ["Create"]);})(smalltalk.send(html, "_button", []));})]);})(smalltalk.send(html, "_form", []));})]);})(smalltalk.send(html, "_div", []));
...
messageSends: ["at:put:", unescape("%2C"), "number", "title", "id:", "with:", "body", "ifTrue:ifFalse:", unescape("%3D"), "size", "asTagBrush:", "asString", "div", "style:", "hr", "strong", "href:", "url", "a", "onClick:", "withIssue:", "new", "appendToJQuery:", "asJQuery", "becomeDialog:", "focus", "loadComments:", "do:", "reversed", "renderComment:onto:"],
